  ...
  const renderList = (items, type) => (
    <ul className="space-y-2">
      {items.map((item) => (
        <li
          key={item.id}
          className={`flex justify-between items-center ${type === "income" ? "bg-green-100 dark:bg-green-800" : "bg-red-100 dark:bg-red-800"} p-2 rounded`}
        >
          ...
          <div>
            <span className="mr-4 dark:text-white">
              {item.amount.toLocaleString("da-DK", { style: "currency", currency: "DKK" })}
            </span>
