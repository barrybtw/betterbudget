...
export const FinanceProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const editItem = (year: string, month: string, item: IncomeExpense) => {
    setFinanceData((prev) => {
      const newData = JSON.parse(JSON.stringify(prev));
      const items = newData[year][month][`${item.type}s`] as IncomeExpense[];
            ...
            Object.keys(newData[y]).forEach((m) => {
              if (y > year || (y === year && m > month)) {
                newData[y][m][`${item.type}s`] = newData[y][m][
                  `${item.type}s`
                ].filter((i: IncomeExpense) => !i.id.startsWith(item.id));
              }
            });
          });
  ...
  const deleteItem = (
    ...
    setFinanceData((prev) => {
      ...
      newData[year][month][`${type}s`] = newData[year][month][
        `${type}s`
      ].filter((item: IncomeExpense) => item.id !== id);
        ...
        Object.keys(newData[y]).forEach((m) => {
          if (y > year || (y === year && m > month)) {
            newData[y][m][`${type}s`] = newData[y][m][`${type}s`].filter(
              (item: IncomeExpense) => !item.id.startsWith(id)
            );
          }
        });
      });
