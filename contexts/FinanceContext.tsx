...
type FinanceContextType = {
  ...
  deleteItem: (year: string, month: string, id: string, type: "income" | "expense") => void
...
export const FinanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addItem = (year: string, month: string, item: IncomeExpense) => {
    setFinanceData((prev) => {
      ...
      if (!newData[year][month]) newData[year][month] = { incomes: [], expenses: [] }
      newData[year][month][`${item.type}s`].push(item)

      // Handle recurring items
      if (item.recurrence !== "once") {
        const monthsToAdd = item.recurrence === "monthly" ? 1 : item.recurrence === "quarterly" ? 3 : 12
        const nextDate = new Date(Number.parseInt(year), Number.parseInt(month) - 1, 1)
        const endDate = item.endDate ? new Date(item.endDate) : new Date(2099, 11, 31) // Set a far future date if no end date
        ...
        while (nextDate <= endDate) {
          ...
          if (!newData[nextYear][nextMonth]) newData[nextYear][nextMonth] = { incomes: [], expenses: [] }
          newData[nextYear][nextMonth][`${item.type}s`].push({ ...item, id: `${item.id}-${nextYear}-${nextMonth}` })
        }
      }
          ...
          Object.keys(newData).forEach((y) => {
            Object.keys(newData[y]).forEach((m) => {
              if (y > year || (y === year && m > month)) {
                newData[y][m][`${item.type}s`] = newData[y][m][`${item.type}s`].filter((i) => !i.id.startsWith(item.id))
              }
            })
          })
  ...
  const deleteItem = (year: string, month: string, id: string, type: "income" | "expense") => {
    setFinanceData((prev) => {
      const newData = JSON.parse(JSON.stringify(prev))
      newData[year][month][`${type}s`] = newData[year][month][`${type}s`].filter((item) => item.id !== id)

      // Handle recurring items
      Object.keys(newData).forEach((y) => {
        Object.keys(newData[y]).forEach((m) => {
          if (y > year || (y === year && m > month)) {
            newData[y][m][`${type}s`] = newData[y][m][`${type}s`].filter((item) => !item.id.startsWith(id))
          }
        })
      })
  ...
  const editGoal = (goal: Goal) => {
    setGoals((prev) => prev.map((g) => (g.id === goal.id ? { ...goal, currentSavings: g.currentSavings } : g)))
  }
